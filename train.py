...
parser.add_argument('--aggr_strategy', type=int, default=0)
parser.add_argument('--lambda_softmax', default=16., type=float, help='Attention softmax temperature.')
...
def load_dataset():
    ...
    _, preprocess_train, preprocess_val = open_clip.create_model_and_transforms('ViT-B-32',
                                                                                pretrained=os.path.join(clip_path,
                                                                                                        'open_clip_pytorch_model.bin'))
    if args.dataset in ['dress', 'shirt', 'toptee']:
        print('Loading FashionIQ-{} dataset'.format(args.dataset))
        fashioniq_dataset = datasets.FashionIQ(path=args.fashioniq_path, category=args.dataset,
                                               transform=[preprocess_train, preprocess_val], split=args.fashioniq_split)
        return [fashioniq_dataset]
    elif args.dataset == 'fashioniq':
        print('Reading fashioniq')
        fashioniq_dataset = datasets.FashionIQ(path=args.fashioniq_path, transform=[preprocess_train, preprocess_val],
...
def create_model_and_optimizer():
    train_model = model.MEDIAN(hidden_dim=args.hidden_dim, dropout=args.dropout_rate, token_num=args.Q, t=args.tau_,
                         aggr_strategy = args.aggr_strategy, lambda_softmax = args.lambda_softmax)
    train_model.cuda()
